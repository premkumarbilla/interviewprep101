...
Live site: https://premkumarbilla.github.io/systemdesignreferences/

Files:
- `index.html`: the whole site. The reference lists are in the HTML for search engines, and JavaScript adds the checkboxes and progress.
- `og-image.png`: the preview image shown when the link is shared.
- `sitemap.xml` and `robots.txt`: help search engines find and index the site.

There's no build step and no server code.
...
The reference data is the `DATA` object in the `<script>` block in `index.html`. The lists are also written into the HTML for search engines, so when you change a reference, update it in both places. If you change the site's address, update the URLs in the `<head>` tags, `sitemap.xml` and `robots.txt` too. Each reference is a `[title, url]` pair, and `url` is `null` for print-only citations.
